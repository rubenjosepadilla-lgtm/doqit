...
export async function POST(req: Request) {
  ...
  await appendEvent(service, doc.id, 'document_signed', `candidate:${candidate.email}`, { manifest_hash: manifestHash, signed_at: now }, null)
  ...
  if (recruiter?.email) {
    await resend.emails.send({
      ...
      html: `
        ...
        <p>Hash del manifiesto: <code>${manifestHash}</code></p>
        <p><a href="${process.env.NEXT_PUBLIC_APP_URL}/dashboard/fes/${doc.id}" style="background:#1d4ed8;color:white;padding:10px 20px;border-radius:8px;text-decoration:none;display:inline-block;margin-top:8px;">Ver documento</a></p>
  ...
  await resend.emails.send({
    ...
    html: `
      ...
      <p>Tu firma electrónica simple ha sido registrada exitosamente para el documento:</p>
      <p><strong>${doc.title}</strong></p>
      <p>Fecha y hora: ${new Date(now).toLocaleString('es-CL')}</p>
      <p style="color:#6b7280;font-size:13px;">Este acto tiene validez legal conforme a la Ley 19.799 de la República de Chile.</p>
      <p style="color:#9ca3af;font-size:12px;margin-top:24px;">Doqit — Firma Electrónica Simple</p>
    `,
  })

  return NextResponse.json({ ok: true, manifestHash })
}
